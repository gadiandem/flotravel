...
@Component({
  selector: 'app-insurance-transaction-detail',
  templateUrl: './insurance-transaction-detail.component.html',
  styleUrls: ['./insurance-transaction-detail.component.css']
})
export class InsuranceTransactionDetailComponent implements OnInit {
  ...
  cancelBooking(statement?: string) {
    ...
    // );
    this.insuranceService
      .cancelPolicy2(this.insuranceDetail.id, statement,'')
      .subscribe(
        (res: FlocashPaymentInsurance) => {
        //  console.log("res: " + JSON.stringify(res));
          this.alertify.success("Cancel booking successful!");
