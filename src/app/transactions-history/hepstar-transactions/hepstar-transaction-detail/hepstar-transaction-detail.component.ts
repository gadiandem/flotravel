...
@Component({
  selector: 'app-hepstar-transaction-detail',
  templateUrl: './hepstar-transaction-detail.component.html',
  styleUrls: ['./hepstar-transaction-detail.component.css']
})
export class HepstarTransactionDetailComponent implements OnInit {
      ...
      if (userId != null) {
        ...
        this.hepstarService.getHepstarHistoryDetail(this.gcaBookingId, req).subscribe(
          (res: FlocashPaymentHepstar) => {
           ...
           //console.log(this.hepstarBookingDetail);
  ...
  cancelBooking(statement?: string) {
    ...
    this.hepstarService.cancelHepstarBooking(this.hepstarBookingDetail.id, statement,this.hepstarBookingDetail.purchaseNumber).subscribe(
      (res: OrderPackageCreateRes) => {
        ...
        this.alertify.warning('Cancellation in progress!');
        this._location.back();
      }, e => {
        this.isCancelling = false;
        console.log(e);
        this.alertify.error(`Could not process request`);
