...
import * as PacakgesActions from '../../store/special-packages.actions';
...
export class PackagePaymentResultComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.sub = this.store.select('specialPackagesList').subscribe((data) => {
      ...
      this.packageSummaryRes = data.packageSummaryRes ||
        JSON.parse(sessionStorage.getItem(specialPackagesConstant.SUMMARY_RESULT));
      this.paymentRes = data.packagesPaymentRes;
      if (this.paymentRes && this.paymentRes.status) {
        this.handlerBookingResult(this.paymentRes);
      }
      this.summaryReq = data.packageSummaryReq ||
        JSON.parse(sessionStorage.getItem(specialPackagesConstant.SUMMARY_REQ));
  ...
  packagesBooking() {
    ...
    if (paymentInfo) {
      const packagesPaymentReq: OrderPackageCreateReq = JSON.parse(
        sessionStorage.getItem(specialPackagesConstant.PACKAGES_PAYMENT_REQ)
      );
      ...
      this.store.dispatch(new PacakgesActions.PaymentPackagesStart({data: packagesPaymentReq}));
    } else {
    ...
    switch (paymentStatus) {
      case FLOCASH_CREATE_ORDER_STATUS['0009']:
        ...
        sessionStorage.setItem(
          appConstant.REDIRECT_SERVICE_NAME,
          bookingRes.serviceName
        );
